...
import {
  ...
  Search, Phone, Mail, ChevronLeft, ChevronRight, X, Check, Link as LinkIcon,
  Share2, Edit3, MapPin
} from "lucide-react";
...
function VenueList({ list, onUpdateList, onRemoveList, onAddHall, onOpenHall, readOnly }) {
  ...
  const filtered = list.halls.filter((h) => h.name.toLowerCase().includes((list.searchQuery || "").toLowerCase()));
  ...
  return (
    <div className="venue-list">
      ...
      <div className="search-bar">
        <Search size={15} />
        <input
          placeholder={`Search ${list.title || "venues"}...`}
          value={list.searchQuery || ""}
          onChange={(e) => onUpdateList({ ...list, searchQuery: e.target.value })}
        />
      </div>
      <div className="venue-grid">
        ...
        {filtered.length === 0 && (
          <div className="empty-note">{list.halls.length === 0 ? "No venues added yet." : "No matches for that search."}</div>
        )}
        {!readOnly && (
          adding ? (
      ...
      <div className="carousel">
        {media.length > 0 ? (
          <>
            ...
            {current.type === "image" ? <img src={current.src} alt="" /> : <video src={current.src} controls />}
            {media.length > 1 && (
...
export default function App() {
  ...
  function handleGenerate() {
    const slug = (siteName || "my-wedding-site").toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "my-wedding-site";
    const suffix = Math.random().toString(36).slice(2, 6);
    setSiteLink(`https://${slug}-${suffix}.wedcraft.site`);
    setPublished(true);
    setView({ type: "home" });
  }
    ...
    <div className="app-root">
      ...
      {published && (
        <div className="publish-banner">
          <Share2 size={16} />
          <span>Your website is live (demo preview): <strong>{siteLink}</strong></span>
