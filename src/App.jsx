...
function BlockRenderer({ block, onUpdate, onRemove, readOnly }) {
  ...
  if (block.type === "image" || block.type === "video") {
      ...
      return (
        <div className="block layout-picker">
          ...
          <div className="layout-choices">
            ...
            <button onClick={() => onUpdate({ ...block, layout: "left" })}>Left side</button>
            <button onClick={() => onUpdate({ ...block, layout: "center" })}>Center</button>
            <button onClick={() => onUpdate({ ...block, layout: "right" })}>Right side</button>
          </div>
    ...
    return (
      <div className={`block block-media layout-${layout}`}>
        {isVideo ? <video src={block.content} autoPlay muted loop playsInline /> : <img src={block.content} alt="" />}
        {!readOnly && (
          <>
  ...
  if (block.type === "mediaText") {
    ...
    return (
      <div className={"block media-text-block" + (block.side === "right" ? " media-text-reverse" : "")}>
        <div className="media-text-media">
          {items.map((it) => (
            <div key={it.id} className="media-text-item">
              {it.kind === "image" ? <img src={it.src} alt="" /> : <video src={it.src} autoPlay muted loop playsInline />}
              {!readOnly && (
                <button className="mt-item-remove" onClick={() => removeItem(it.id)} aria-label="Remove"><X size={10} /></button>
              )}
            </div>
          ))}
          {items.length === 0 && readOnly && <div className="media-text-empty">No photos or videos added yet.</div>}
          {!readOnly && (
...
function CategoryPage({ path, categories, setCategories, goHome, goToCategory, goToHall, published }) {
  ...
  const addMediaTextBlock = (side) => update((c) => ({ ...c, blocks: [...c.blocks, { id: uid(), type: "mediaText", side, items: [], text: "" }] }));
...
export default function App() {
  ...
  const addMainMediaTextBlock = (side) => setMainBlocks((b) => [...b, { id: uid(), type: "mediaText", side, items: [], text: "" }]);
...
function GlobalStyles() {
  return (
    <style>{`
      ...
      .block-media img, .block-media video { width: 100%; display: block; object-fit: cover; }
      ...
      .block-media.layout-fill img, .block-media.layout-fill video { height: min(58vh, 520px); }
      .block-media.layout-left { width: 60%; margin-right: auto; }
      .block-media.layout-right { width: 60%; margin-left: auto; }
      .block-media.layout-center { width: 66%; margin: 0 auto; }
      .block-media.layout-left img, .block-media.layout-left video,
      .block-media.layout-right img, .block-media.layout-right video,
      .block-media.layout-center img, .block-media.layout-center video {
        max-height: 420px;
      }
      ...
      .media-text-block { display: flex; gap: 20px; align-items: flex-start; }
      .media-text-block.media-text-reverse { flex-direction: row-reverse; }
      .media-text-media { flex: 1; display: flex; flex-wrap: wrap; gap: 10px; align-content: flex-start; min-width: 0; }
      .media-text-item { position: relative; width: calc(50% - 5px); border-radius: 6px; overflow: hidden; background: var(--ivory); }
      .media-text-item img, .media-text-item video { width: 100%; height: 140px; object-fit: cover; display: block; }
      .mt-item-remove {
      ...
      .media-text-empty { width: 100%; padding: 30px; text-align: center; color: var(--muted); font-size: 13px; border: 1px dashed var(--border); border-radius: 6px; }
      .media-text-add { display: flex; flex-direction: column; gap: 6px; width: calc(50% - 5px); }
      .media-text-add .file-choose { padding: 16px 10px; height: 100%; }
      .media-text-para { flex: 1; font-size: 14.5px; line-height: 1.75; margin: 0; white-space: pre-wrap; min-width: 0; }
      .media-text-textarea { flex: 1; min-height: 160px; font-size: 14.5px; line-height: 1.7; resize: vertical; min-width: 0; }
      @media (max-width: 640px) {
        .media-text-block, .media-text-block.media-text-reverse { flex-direction: column; }
        .block-media.layout-left, .block-media.layout-right, .block-media.layout-center { width: 100%; }
      }
