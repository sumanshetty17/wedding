...
import {
  Star, Phone, Mail, ChevronDown, ChevronLeft, ChevronRight, X, Play,
...
import { EMAILJS_CONFIG, isEmailConfigured } from "./emailConfig.js";
import theBeginningLogo from "./assets/partners/the-beginning-logo.png";

// Loads every photo/video under src/assets/venues/<venue-id>/<section>/*
// automatically — to add a new venue's real gallery, just create that
// folder structure and drop files in; no code changes needed here.
const venueGalleryFiles = import.meta.glob("./assets/venues/*/*/*.{jpg,jpeg,png,mp4}", {
  eager: true,
  import: "default",
});
...
function getVenueGallery(venueId) {
  const sections = {};
  for (const path in venueGalleryFiles) {
    const match = path.match(/assets\/venues\/([^/]+)\/([^/]+)\/[^/]+$/);
    if (!match) continue;
    const [, vId, section] = match;
    if (vId !== venueId) continue;
    if (!sections[section]) sections[section] = [];
    sections[section].push({
      src: venueGalleryFiles[path],
      type: isVideoPath(path) ? "video" : "image",
      key: path,
    });
  }
  // Stable, sensible order regardless of filesystem order
  const order = ["venue", "event", "decor", "rooms", "surroundings"];
  const byKey = (a, b) => (a.key < b.key ? -1 : 1);
  return order
    .filter((s) => sections[s] && sections[s].length)
    .map((s) => ({ id: s, label: SECTION_LABELS[s] || s, items: sections[s].sort(byKey) }))
    .concat(
...
// Short trust badges shown as a row under the hero heading. Kept
// qualitative rather than invented numbers (like "500+ weddings") —
// swap these for real stats once you have them.
const HERO_BADGES = [
  { label: "Handpicked", sub: "Venues" },
  { label: "One Team", sub: "Start to Finish" },
  { label: "Clear", sub: "Pricing" },
];
const VALUE_PROPS = [
...
const CATEGORIES = [
  {
    ...
    // Real venues go here as we onboard them — no placeholders.
    venues: [],
  },
  {
    id: "outdoor",
    ...
    venues: [
      {
        ...
        name: "The Beginning",
        rating: 4.8,
        priceRange: "₹6,00,000+ (plus tax) for a 24-hour booking",
        photos: [
          // TEMP: pulled from their own public website as a placeholder.
          // Swap these for the real photos/video they send us directly —
          // just replace the URLs below (or point to local files, see note
          // at the top of this file for how that works).
          "https://static.wixstatic.com/media/6b6cda_dc0820f1fb17461a9fa202ad0b22d99a~mv2.jpg",
          "https://static.wixstatic.com/media/6b6cda_5d6bec2de7e8470396f08add64abe9bf~mv2.jpeg",
          "https://static.wixstatic.com/media/6b6cda_e1ea8e166d8d49c295cac218610bc55d~mv2.jpeg",
        ],
        videos: [],
        details:
          "A 3-acre boutique wedding resort on Sarjapur Road, hosting 350+ weddings since 2020. Accommodates up to 750 guests in the lawn and up to 750 in the banquet hall, with a 7-star bridal suite, 15 luxury guest rooms, and parking for 300+ cars — a self-contained wedding destination just outside the city.",
      },
    ],
  },
  {
...
// Real venue-partner logos for the sliding strip on the home page.
// Add a new entry here (name + imported logo) as each new partner signs on.
const PARTNER_LOGOS = [
  { name: "The Beginning", logo: theBeginningLogo },
];
...
function QuoteModal({ onClose }) {
  ...
  async function handleSubmit() {
    ...
    if (!isEmailConfigured) {
      setSubmitError("Enquiry delivery isn't set up yet — this site needs EmailJS connected first. See README.md.");
      return;
    }
    ...
    try {
      ...
      setSubmitError("Something went wrong sending your enquiry. Please try again in a moment.");
    } finally {
      ...
      <div className="quote-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {step === "form" && (
          <>
            ...
            {submitError && <p className="quote-otp-error">{submitError}</p>}

            <button
              className={"quote-submit" + (isFormValid && !isSubmitting ? " ready" : "")}
        ...
        {step === "thanks" && (
          <div className="quote-thanks">
            ...
            <button className="quote-submit ready" onClick={onClose}>Done</button>
          </div>
        )}

        <div className="quote-modal-brand">
          <img src={logoImg} alt="MarriHeaven logo" className="quote-modal-logo" />
          MARRIHEAVEN
        </div>
      </div>
    </div>
          ...
          {HERO_BADGES.map((b, i) => (
            <React.Fragment key={b.label}>
              ...
              <div className="hero-stat">
                <span className="hero-stat-label">{b.label}</span>
                <span className="hero-stat-sub">{b.sub}</span>
              </div>
            </React.Fragment>
          ))}
        </div>
...
function QuoteFeature() {
  return (
    <div className="section-band-outer">
      <WaveTop color="#D6336C" />
      <div className="quote-feature-band">
...
// A soft, subtle line-art floral decoration for quiet corners of a
// section — vector-based (so it's always crisp, never blurry) and
// meant to sit at low opacity behind real content.
function FloralCorner(props) {
  return (
    <svg {...props} viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <g stroke="currentColor" strokeWidth="1.4">
        <path d="M20,20 C70,10 110,40 120,90 C130,140 170,160 220,150" />
        <path d="M40,45 C55,35 72,38 80,55 C88,72 78,88 60,90 C42,92 30,78 32,60 C33,52 36,48 40,45 Z" />
        <path d="M85,95 C100,85 117,88 125,105 C133,122 123,138 105,140 C87,142 75,128 77,110 C78,102 81,98 85,95 Z" />
        <path d="M150,135 C165,125 182,128 190,145 C198,162 188,178 170,180 C152,182 140,168 142,150 C143,142 146,138 150,135 Z" />
        <path d="M20,20 C30,45 28,70 12,90" />
        <path d="M220,150 C205,160 195,175 195,195" />
      </g>
    </svg>
  );
}
...
function SectionBand({ color, children }) {
  return (
    <div className="section-band-outer">
      <WaveTop color={color} />
...
// A continuously sliding strip of partner venue logos. The list is
// duplicated once so the CSS animation can loop seamlessly — as real
// partners are added to PARTNER_LOGOS above, they just join the loop.
function PartnerLogoStrip() {
  const loopLogos = [...PARTNER_LOGOS, ...PARTNER_LOGOS, ...PARTNER_LOGOS, ...PARTNER_LOGOS];
  return (
    <section className="partner-strip-section">
      <h2>Venues We're Proud to Partner With</h2>
      <p className="partner-strip-sub">A growing circle of real properties across Bangalore — and counting.</p>
      <div className="partner-strip-track-wrap">
        <div className="partner-strip-track">
          {loopLogos.map((p, i) => (
            <div className="partner-strip-logo" key={p.name + i}>
...
function HomePage({ goToCategory }) {
  ...
  return (
    <div className="page home-page">
      ...
      <SectionBand color="var(--band-gold)">
        <ValueProps />
      </SectionBand>
      ...
      <SectionBand color="var(--band-blush)">
      ...
      <section className="services-section" id="services">
        <FloralCorner className="floral-deco floral-deco-tl" />
        <FloralCorner className="floral-deco floral-deco-tr" />
        <FloralCorner className="floral-deco floral-deco-bl" />
        <FloralCorner className="floral-deco floral-deco-br" />
...
function CategoryPage({ categoryId, goToVenue }) {
  ...
  return (
    <div className="page category-page">
      <div className="category-hero-band">
        <h1>{category.name}</h1>
      ...
      {category.venues.length === 0 ? (
        <div className="venue-list-empty">
          <Sparkles size={28} />
          <p>We're onboarding real {category.name.toLowerCase()} venues right now — check back soon, or reach out and we'll help you find one directly.</p>
        </div>
      ) : (
...
// Tabbed photo gallery (Venue / Decor / Rooms / Surroundings, etc.) shown
// on a venue's page once it has real categorized photos. Click a section
// pill to switch the grid; click any photo to open it full-size.
function VenueSectionGallery({ sections }) {
  const [activeId, setActiveId] = useState(sections[0].id);
  const [lightboxIndex, setLightboxIndex] = useState(null);
  const active = sections.find((s) => s.id === activeId) || sections[0];

  useEffect(() => { setLightboxIndex(null); }, [activeId]);

  function closeLightbox() { setLightboxIndex(null); }
  function nextPhoto(e) {
    e.stopPropagation();
    setLightboxIndex((i) => (i + 1) % active.items.length);
  }
  function prevPhoto(e) {
    e.stopPropagation();
    setLightboxIndex((i) => (i - 1 + active.items.length) % active.items.length);
  }

  const lightboxItem = lightboxIndex !== null ? active.items[lightboxIndex] : null;

  return (
    <div className="venue-gallery">
      <div className="venue-gallery-tabs">
        {sections.map((s) => (
          <button
            key={s.id}
            className={"venue-gallery-tab" + (s.id === activeId ? " active" : "")}
            onClick={() => setActiveId(s.id)}
          >
            {s.label} <span className="venue-gallery-tab-count">{s.items.length}</span>
          </button>
        ))}
      </div>
      ...
      <div className="venue-gallery-grid">
        {active.items.map((item, i) => (
          <button
            key={item.key}
            type="button"
            className="venue-gallery-thumb"
            onClick={() => setLightboxIndex(i)}
          >
            {item.type === "video" ? (
              <>
                <video src={item.src} muted playsInline preload="metadata" />
                <span className="venue-gallery-play"><Play size={18} fill="#fff" /></span>
              </>
            ) : (
              <img src={item.src} alt={`${active.label} ${i + 1}`} loading="lazy" />
            )}
          </button>
        ))}
      </div>
      ...
      {lightboxItem && (
        <div className="venue-lightbox" onClick={closeLightbox}>
          <button className="venue-lightbox-close" onClick={closeLightbox} aria-label="Close"><X size={22} /></button>
          <button className="venue-lightbox-nav venue-lightbox-prev" onClick={prevPhoto} aria-label="Previous"><ChevronLeft size={26} /></button>
          ...
          <button className="venue-lightbox-nav venue-lightbox-next" onClick={nextPhoto} aria-label="Next"><ChevronRight size={26} /></button>
        </div>
      )}
    </div>
  );
...
function VenuePage({ categoryId, venueId }) {
  ...
  const gallerySections = getVenueGallery(venue.id);
  ...
  return (
    <div className="page venue-page">
      ...
      {gallerySections.length > 0 && (
        <div className="content-card venue-gallery-card">
          <h2 className="venue-gallery-heading">Explore {venue.name}</h2>
          <VenueSectionGallery sections={gallerySections} />
        </div>
      )}
    </div>
  );
...
function GlobalStyles() {
  return (
    <style>{`
      ...
      .app-root {
        /* Clean white base with a warm rose + gold accent pair.
           No stretched background photos anywhere — every texture here
           is a solid color or a crisp vector curve, so it never blurs. */
        --ivory: #FFFDFB;
        --surface: #FFFFFF;
        --wine: #D6336C;
        --wine-dark: #A61E52;
      ...
      .site-header {
        ...
        box-shadow: 0 2px 16px rgba(36,21,33,0.06);
      ...
      .brand-tagline { font-size: 12px; color: var(--muted); letter-spacing: 0.2px; }
      .main-nav { display: flex; align-items: center; gap: 8px; }
      .nav-link {
        font-family: 'Poppins', sans-serif; font-size: 14px; font-weight: 500;
        color: var(--charcoal); padding: 10px 18px; border-radius: 30px;
      }
      .nav-link:hover, .nav-link.active { color: var(--wine); background: var(--gold-light); }

      .site-main { max-width: 1080px; margin: 0 auto; padding: 48px 24px 60px; }
      .page h1 {
        font-family: 'Poppins', sans-serif; font-weight: 800; font-size: 34px; color: var(--ink);
        text-align: center; margin: 0 0 6px; letter-spacing: -0.5px;
      }

      .ornament {
        width: 44px; height: 4px; margin: 14px auto 18px; border-radius: 4px;
        background: linear-gradient(90deg, var(--wine), var(--gold));
      }
      .ornament.small { width: 36px; margin: 0 auto 16px; }
      ...
      .hero-overlay h1 {
        font-family: 'Poppins', sans-serif; font-weight: 800; font-size: 48px; color: #fff;
        margin: 0 0 14px; letter-spacing: -1px; max-width: 720px; line-height: 1.15;
        text-shadow: 0 2px 20px rgba(0,0,0,0.35);
      }
      .hero-overlay p { font-family: 'Inter', sans-serif; font-size: 17px; color: rgba(255,255,255,0.88); max-width: 480px; margin: 0 0 30px; line-height: 1.6; }
      .app-root .hero-cta {
        background: rgba(214,51,108,0.32); color: #fff; border: 1px solid rgba(255,255,255,0.4);
        backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px);
        border-radius: 30px; padding: 15px 34px; font-size: 15px; font-weight: 600; letter-spacing: 0.2px;
        box-shadow: 0 10px 26px rgba(214,51,108,0.2);
        transition: background 0.15s ease;
      }
      .app-root .hero-cta:hover { background: rgba(166,30,82,0.85); }
      .hero-stats {
        display: flex; align-items: center; gap: 20px; margin-top: 40px;
        background: rgba(255,255,255,0.12); backdrop-filter: blur(6px);
        border: 1px solid rgba(255,255,255,0.25); border-radius: 20px; padding: 16px 28px;
      }
      .hero-stat { display: flex; flex-direction: column; align-items: center; gap: 2px; }
      .hero-stat-label { font-family: 'Poppins', sans-serif; font-weight: 700; font-size: 16px; color: #fff; }
      .hero-stat-sub { font-family: 'Inter', sans-serif; font-size: 11.5px; color: rgba(255,255,255,0.75); letter-spacing: 0.2px; }
      .hero-stats-divider { width: 1px; height: 30px; background: rgba(255,255,255,0.3); }
      ...
      .value-props-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 24px; }
      .value-prop { text-align: center; padding: 10px 14px; }
      .value-prop-icon {
        width: 56px; height: 56px; border-radius: 50%; background: var(--surface); color: var(--wine);
        display: flex; align-items: center; justify-content: center; margin: 0 auto 14px;
        box-shadow: 0 8px 20px rgba(214,51,108,0.15);
      }
      .value-prop h3 { font-family: 'Poppins', sans-serif; font-weight: 600; font-size: 17px; color: var(--ink); margin: 0 0 6px; }
      ...
      .process-section h2 { font-family: 'Poppins', sans-serif; font-weight: 800; font-size: 30px; color: var(--ink); letter-spacing: -0.5px; }
      .process-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 24px; margin-top: 14px; }
      .process-step {
        position: relative; border-radius: 16px; padding: 32px 20px 22px; background: var(--surface);
        box-shadow: 0 8px 24px rgba(36,21,33,0.07);
      }
      .process-step-number {
      ...
      .process-step-icon { color: var(--wine); margin: 6px 0 12px; display: flex; justify-content: center; }
      .process-step h3 { font-family: 'Poppins', sans-serif; font-weight: 600; font-size: 16px; color: var(--ink); margin: 0 0 8px; }
      ...
      .inspiration-section h2 { font-family: 'Poppins', sans-serif; font-weight: 800; font-size: 30px; color: var(--ink); letter-spacing: -0.5px; }
      ...
      .inspiration-thumb {
        ...
        background: var(--ivory); box-shadow: 0 6px 18px rgba(36,21,33,0.1);
        display: flex; align-items: center; justify-content: center;
      }
      .inspiration-thumb img { width: 100%; height: 100%; object-fit: cover; display: block; }
      .inspiration-more { background: var(--wine); color: #fff; }
      .inspiration-item span { font-family: 'Poppins', sans-serif; font-size: 12.5px; font-weight: 500; color: var(--charcoal); }
      .app-root .inspiration-view-all {
        display: inline-flex; align-items: center; gap: 6px;
        border: 1px solid rgba(214,51,108,0.35); color: var(--wine); background: rgba(214,51,108,0.08); border-radius: 30px;
        backdrop-filter: blur(6px); -webkit-backdrop-filter: blur(6px);
        padding: 12px 26px; font-size: 13.5px; font-weight: 600;
        box-shadow: 0 8px 20px rgba(214,51,108,0.12);
        transition: background 0.15s ease, color 0.15s ease;
      }
      .app-root .inspiration-view-all:hover { background: var(--wine); color: #fff; }
      .app-root .inspiration-view-all:hover .inspiration-view-all-icon { color: #fff; }
      .inspiration-view-all-icon { margin-top: 1px; }

      /* faq */
      .faq-section { max-width: 680px; margin: 0 auto 50px; text-align: center; }
      .faq-section h2 { font-family: 'Poppins', sans-serif; font-weight: 800; font-size: 30px; color: var(--ink); letter-spacing: -0.5px; }
      .faq-list { text-align: left; display: flex; flex-direction: column; gap: 12px; margin-top: 14px; }
      .faq-item { border-radius: 14px; background: var(--surface); overflow: hidden; box-shadow: 0 4px 16px rgba(36,21,33,0.06); }
      .faq-question {
        width: 100%; display: flex; align-items: center; justify-content: space-between; gap: 10px;
        padding: 16px 20px; font-family: 'Poppins', sans-serif; font-size: 14.5px; font-weight: 500; color: var(--charcoal); text-align: left;
      }
      .faq-chevron { color: var(--wine); flex-shrink: 0; transition: transform 0.2s ease; }
      .faq-item.open .faq-chevron { transform: rotate(180deg); }
      .faq-answer { padding: 0 20px 18px; font-size: 14px; line-height: 1.7; color: var(--muted); margin: 0; }
      ...
      .band-wave { display: block; width: 100%; height: 60px; margin-bottom: -2px; }
      .section-band { padding: 60px 24px; }
      .section-band-inner { max-width: 1080px; margin: 0 auto; }
      ...
      .quote-feature-text {
        ...
        color: #FFFFFF; max-width: 640px; margin: 0 auto; line-height: 1.6;
      }
      @media (max-width: 640px) {
        .quote-feature-band { padding: 60px 20px; }
        .quote-feature-text { font-size: 20px; }
      }

      /* page-level backdrops (About / Contact) — a soft, subtle line-art
         floral touch in the corners, vector-based so it's always crisp */
      ...
      .floral-deco-tl { top: -30px; left: -40px; width: 240px; height: 240px; }
      .floral-deco-tr { top: -50px; right: -60px; width: 190px; height: 190px; transform: rotate(90deg); opacity: 0.3; }
      .floral-deco-bl { bottom: -50px; left: -60px; width: 190px; height: 190px; transform: rotate(-90deg); opacity: 0.3; }
      .floral-deco-br { bottom: -30px; right: -40px; width: 240px; height: 240px; transform: rotate(180deg); }
      @media (max-width: 640px) {
        .floral-deco-tl, .floral-deco-br { width: 150px; height: 150px; }
        .floral-deco-tr, .floral-deco-bl { width: 110px; height: 110px; }
      }
      ...
      .category-hero-band {
        ...
        background: linear-gradient(135deg, var(--wine) 0%, var(--wine-dark) 100%);
        border-radius: 24px;
        padding: 50px 24px 42px; margin: 0 0 34px;
      }
      .category-hero-band h1 { color: #fff; margin-bottom: 8px; }
      .category-hero-band .category-tagline { color: rgba(255,255,255,0.85); margin: 0; }

      .story-section { display: flex; flex-direction: column; gap: 24px; margin-bottom: 50px; }
      .story-block {
        display: flex; align-items: center; gap: 30px;
        border-radius: 20px; overflow: hidden; background: var(--surface);
        box-shadow: 0 8px 28px rgba(36,21,33,0.08);
      }
      ...
      /* partner logo sliding strip */
      .partner-strip-section { text-align: center; padding: 10px 0 46px; }
      .partner-strip-section h2 {
        font-family: 'Poppins', sans-serif; font-weight: 800; font-size: 26px; color: var(--ink);
        letter-spacing: -0.5px; margin: 0 0 6px;
      }
      .partner-strip-sub { font-size: 13.5px; color: var(--muted); margin: 0 0 26px; }
      ...
      .partner-strip-track {
        ...
        animation: partner-strip-scroll 22s linear infinite;
      }
      .partner-strip-logo { flex-shrink: 0; height: 46px; display: flex; align-items: center; }
      .partner-strip-logo img { height: 100%; width: auto; max-width: 160px; object-fit: contain; opacity: 0.85; }
      @keyframes partner-strip-scroll {
        from { transform: translateX(0); }
        to { transform: translateX(-25%); }
      }
      @media (max-width: 640px) {
        .partner-strip-track { gap: 40px; }
        .partner-strip-logo { height: 36px; }
      }

      /* empty category state (before real venues are onboarded) */
      .venue-list-empty {
        max-width: 420px; margin: 0 auto; text-align: center; padding: 50px 20px;
        color: var(--muted);
      }
      .venue-list-empty svg { color: var(--gold); margin-bottom: 12px; }
      .venue-list-empty p { font-size: 14.5px; line-height: 1.7; margin: 0; }

      .services-section { position: relative; overflow: hidden; border-radius: 24px; padding: 10px 0; }
      .services-section .section-content { position: relative; z-index: 1; }
      .categories-section h2, .services-section h2 {
        font-family: 'Poppins', sans-serif; font-weight: 800; font-size: 30px; color: var(--ink); letter-spacing: -0.5px;
      }
      .category-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 20px; margin-top: 14px; }
      .app-root .category-card {
        border-radius: 18px; overflow: hidden; background: var(--surface);
        text-align: left; box-shadow: 0 8px 24px rgba(36,21,33,0.08);
        transition: transform 0.15s ease, box-shadow 0.15s ease;
      }
      .app-root .category-card:hover { transform: translateY(-3px); box-shadow: 0 14px 32px rgba(36,21,33,0.14); }
      .category-card-img { height: 180px; overflow: hidden; }
      .category-card-img img { width: 100%; height: 100%; object-fit: cover; display: block; }
      .category-card-label {
        padding: 16px; font-family: 'Poppins', sans-serif; font-weight: 600; font-size: 16px; color: var(--ink); text-align: center;
      }

      .services-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 18px; margin-top: 14px; }
      .service-card {
        border-radius: 16px; padding: 26px 20px; background: var(--surface);
        box-shadow: 0 6px 20px rgba(36,21,33,0.07);
      }
      .service-icon {
        width: 52px; height: 52px; border-radius: 50%; background: var(--ivory); color: var(--wine);
        display: flex; align-items: center; justify-content: center; margin: 0 auto 14px;
        box-shadow: 0 6px 16px rgba(214,51,108,0.15);
      }
      .service-card h3 { font-family: 'Poppins', sans-serif; font-weight: 600; font-size: 16px; color: var(--ink); margin: 0 0 8px; }
      .service-card p { font-size: 13.5px; line-height: 1.6; color: var(--muted); margin: 0; }

      .trust-section p { max-width: 640px; margin: 0 auto 14px; font-size: 15px; line-height: 1.8; color: var(--charcoal); }
      .trust-section p:last-child { margin-bottom: 0; }

      /* footer */
      .site-footer {
        background: linear-gradient(160deg, #2B1420 0%, #1A0C13 100%);
        padding: 40px 24px 110px; text-align: center;
        display: flex; flex-direction: column; align-items: center; gap: 10px;
      }
      .footer-row, .site-footer > .ornament { position: relative; }
      .footer-row { display: flex; align-items: center; gap: 8px; font-size: 14px; color: var(--gold-light); }
      .footer-row a { color: #fff; }
      .site-footer .muted { color: rgba(253,238,220,0.6); }
      ...
      .content-card {
        ...
        box-shadow: 0 10px 32px rgba(36,21,33,0.08);
      }
      ...
      .app-root .venue-row-card {
        display: flex; flex-direction: column; text-align: left;
        border-radius: 16px; overflow: hidden; background: var(--surface);
        box-shadow: 0 8px 24px rgba(36,21,33,0.08);
        transition: transform 0.15s ease;
      }
      .app-root .venue-row-card:hover { transform: translateY(-2px); }
      ...
      .venue-row-name {
        padding: 14px 16px; font-family: 'Poppins', sans-serif; font-size: 19px; color: var(--wine-dark);
      }
      ...
      /* venue section gallery (Venue / Decor / Rooms / Surroundings) */
      .venue-gallery-card { margin-top: 20px; padding: 22px 20px; text-align: left; }
      .venue-gallery-heading {
        font-family: 'Poppins', sans-serif; font-weight: 700; font-size: 19px; color: var(--ink);
        margin: 0 0 16px;
      }
      .venue-gallery-tabs { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 16px; }
      .app-root .venue-gallery-tab {
        border: 1px solid var(--border); background: var(--ivory); color: var(--charcoal);
        border-radius: 20px; padding: 8px 16px; font-size: 13.5px; font-weight: 500;
        display: inline-flex; align-items: center; gap: 6px;
      }
      .app-root .venue-gallery-tab.active { background: var(--wine); border-color: var(--wine); color: #fff; }
      .venue-gallery-tab-count {
        font-size: 11px; opacity: 0.75; background: rgba(0,0,0,0.08); border-radius: 10px; padding: 1px 6px;
      }
      .venue-gallery-tab.active .venue-gallery-tab-count { background: rgba(255,255,255,0.25); }

      .venue-gallery-grid {
        display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 10px;
      }
      .app-root .venue-gallery-thumb {
        position: relative;
        border: none; border-radius: 10px; overflow: hidden; padding: 0; background: var(--ivory);
        aspect-ratio: 4 / 3; cursor: zoom-in;
      }
      .venue-gallery-thumb img, .venue-gallery-thumb video {
        width: 100%; height: 100%; object-fit: cover; display: block; transition: transform 0.2s ease;
      }
      .venue-gallery-thumb:hover img, .venue-gallery-thumb:hover video { transform: scale(1.05); }
      .venue-gallery-play {
        position: absolute; inset: 0; display: flex; align-items: center; justify-content: center;
        background: rgba(20,10,15,0.28); pointer-events: none;
      }
      .venue-gallery-play svg { filter: drop-shadow(0 2px 6px rgba(0,0,0,0.5)); }

      .venue-lightbox {
        position: fixed; inset: 0; background: rgba(20,10,15,0.92); z-index: 400;
        display: flex; align-items: center; justify-content: center; padding: 40px 20px;
      }
      .venue-lightbox-img { max-width: 100%; max-height: 88vh; border-radius: 6px; object-fit: contain; }
      .app-root .venue-lightbox-close {
        position: absolute; top: 18px; right: 18px; color: #fff; background: rgba(255,255,255,0.12);
        border: none; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center;
      }
      .app-root .venue-lightbox-nav {
        position: absolute; top: 50%; transform: translateY(-50%); color: #fff; background: rgba(255,255,255,0.12);
        border: none; width: 44px; height: 44px; border-radius: 50%; display: flex; align-items: center; justify-content: center;
      }
      .venue-lightbox-prev { left: 16px; }
      .venue-lightbox-next { right: 16px; }
      @media (max-width: 640px) {
        .venue-gallery-grid { grid-template-columns: repeat(auto-fill, minmax(120px, 1fr)); }
        .venue-lightbox-nav { width: 38px; height: 38px; }
      }

      /* floating CTA + quote modal */
      .app-root .floating-cta {
        position: fixed; bottom: 22px; left: 0; right: 0; margin: 0 auto;
        width: fit-content; max-width: calc(100vw - 40px);
        background-color: rgba(214,51,108,0.55);
        backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);
        color: #ffffff;
        border: 1px solid rgba(255,255,255,0.35);
        border-radius: 30px;
        padding: 15px 28px; font-size: 14px; font-weight: 600; letter-spacing: 0.3px;
        box-shadow: 0 10px 30px rgba(214,51,108,0.22); z-index: 200;
        transition: background-color 0.15s ease;
      }
      .app-root .floating-cta:hover { background-color: rgba(166,30,82,0.88); }
      ...
      .quote-modal h2 {
        font-family: 'Poppins', sans-serif; font-weight: 700; font-size: 21px; color: var(--wine-dark);
        margin: 6px 0 8px;
      }
      ...
      .app-root .quote-submit {
        width: 100%; border: none; border-radius: 30px; padding: 14px;
        font-size: 14.5px; font-weight: 600; color: #fff; background: #b9b0a8;
        transition: box-shadow 0.2s ease;
      }
      .app-root .quote-submit.ready {
        background: rgba(214,51,108,0.7); border: 1px solid rgba(255,255,255,0.35);
        backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px);
        box-shadow: 0 8px 22px rgba(138,31,56,0.2);
      }
      .app-root .quote-submit.ready:hover { background: var(--wine-dark); }
      ...
      .quote-thanks-icon { color: #2E9E5B; margin-bottom: 12px; }
      ...
      .quote-modal-logo { width: 26px; height: 26px; object-fit: contain; }
