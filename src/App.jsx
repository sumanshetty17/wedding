        ...
        {step === "form" && (
          <>
            ...
            <div className="quote-select-wrap">
              ...
              >
                <option value="" disabled>Tell us your wedding type</option>
                {WEDDING_TYPES.map((t) => (
                  <option key={t} value={t}>{t}</option>
                ))}
              </select>
              <ChevronDown size={16} className="quote-select-chevron" />
            </div>
  ...
  return (
    <style>{`
      ...
      .floating-cta {
        ...
        box-shadow: 0 10px 26px rgba(194,37,92,0.35); z-index: 200; opacity: 1;
      }
      ...
      .quote-select {
        width: 100%; background: var(--ivory); border: 1px solid var(--border); border-radius: 8px;
        padding: 13px 38px 13px 14px; font-size: 14px; color: var(--charcoal);
        appearance: none; -webkit-appearance: none; -moz-appearance: none;
      }
      .quote-select:invalid { color: var(--muted); }
      .quote-select:focus { outline: none; border-color: var(--gold); }
