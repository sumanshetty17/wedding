...
function BlockRenderer({ block, onUpdate, onRemove, readOnly }) {
  ...
  if (block.type === "image" || block.type === "video") {
    ...
    return (
      <div className="block-media-wrap">
        <div className="block block-media layout-custom" style={{ marginLeft: `${(hpos / 100) * 40}%` }}>
          {mediaEl}
          {!readOnly && (
            <>
              <button className="change-layout-btn" onClick={() => onUpdate({ ...block, layout: null })} aria-label="Change layout"><Edit3 size={12} /></button>
...
function GlobalStyles() {
  return (
    <style>{`
      ...
      .block-media-wrap { display: flex; flex-direction: column; }
      .block-media.layout-custom { width: 60%; }
      .hpos-controls { display: flex; align-items: center; gap: 8px; margin-top: 8px; }
