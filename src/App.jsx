...
// Friendly text for the Firebase Auth error codes we're most likely
// to hit, so people never see a raw "auth/xyz" code.
function firebaseAuthErrorMessage(err) {
  const code = err && err.code;
  if (code === "auth/invalid-phone-number") return "That phone number doesn't look right. Please check it and try again.";
  if (code === "auth/too-many-requests") return "Too many attempts from this number. Please wait a bit and try again.";
  if (code === "auth/quota-exceeded") return "We've hit today's SMS limit — please try again later or call us directly.";
  if (code === "auth/captcha-check-failed") return "Verification check failed. Please refresh the page and try again.";
  if (code === "auth/code-expired") return "That code expired. Tap \"Resend OTP\" for a new one.";
  if (code === "auth/invalid-verification-code") return "That code doesn't match. Please enter the correct OTP.";
  return "Something went wrong sending the code. Please try again in a moment.";
}
...
function QuoteModal({ onClose }) {
  ...
  // Clean up the invisible reCAPTCHA widget when the modal closes.
  useEffect(() => {
    return () => {
  ...
  async function requestOtp() {
    if (!isFirebaseConfigured) {
      setSendError("OTP verification isn't set up yet — this site needs a Firebase project connected first. See README.md.");
      return;
    }
    setSendError("");
    setIsSending(true);
    try {
      // A fresh verifier each send keeps this simple and reliable —
      // reCAPTCHA widgets don't like being reused after a failure.
      if (recaptchaVerifierRef.current) {
        recaptchaVerifierRef.current.clear();
      }
      recaptchaVerifierRef.current = new RecaptchaVerifier(auth, "recaptcha-container", { size: "invisible" });
      const confirmation = await signInWithPhoneNumber(auth, "+91" + phone.trim(), recaptchaVerifierRef.current);
      ...
      setSendError(firebaseAuthErrorMessage(err));
      return false;
    } finally {
      setIsSending(false);
    }
  ...
  async function handleVerify() {
    ...
    if (!confirmationRef.current) {
      setOtpError("That code expired. Tap \"Resend OTP\" for a new one.");
      return;
    }
    setIsVerifying(true);
    setOtpError("");
  ...
  return (
    <div className="quote-modal-backdrop" onClick={onClose}>
      <div className="quote-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {/* Invisible reCAPTCHA anchor Firebase needs for phone auth — not shown to the person */}
        <div id="recaptcha-container" className="recaptcha-anchor" />
        ...
        {step === "form" && (
          <>
            ...
            {sendError && <p className="quote-otp-error">{sendError}</p>}

            <button
              className={"quote-submit" + (isFormValid && !isSending ? " ready" : "")}
              disabled={!isFormValid || isSending}
              onClick={handleSendOtp}
            >
              {isSending ? "Sending code…" : "Submit"}
            </button>
          </>
        )}
          ...
          <>
            ...
            <p className="quote-resend">
              {resendTimer > 0 ? (
                ...
                <button type="button" className="quote-resend-link" onClick={requestOtp}>Resend OTP</button>
              )}
            </p>

            <button className={"quote-submit" + (isVerifying ? "" : " ready")} disabled={isVerifying} onClick={handleVerify}>
...
function GlobalStyles() {
  return (
    <style>{`
      ...
      .recaptcha-anchor { position: absolute; width: 0; height: 0; overflow: hidden; }
      .quote-input {
