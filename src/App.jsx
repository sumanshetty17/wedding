      ...
      <div className="quote-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {step === "form" && (
          <>
            ...
            <p className="quote-modal-sub">Share a few details so our team can get in touch with you.</p>

            <input className="quote-input" placeholder="Enter your name" value={name} onChange={(e) => setName(e.target.value)} />
            ...
            <input className="quote-input" placeholder="Tell us your event location" value={location} onChange={(e) => setLocation(e.target.value)} />

            <div className="quote-type-label">Tell us your wedding type</div>
            <div className="quote-type-options">
              {WEDDING_TYPES.map((t) => (
                <button
                  key={t}
                  type="button"
                  className={"quote-type-chip" + (weddingType === t ? " active" : "")}
                  onClick={() => setWeddingType(t)}
                >
                  {t}
                </button>
              ))}
  ...
  return (
    <style>{`
      ...
      .floating-cta {
        ...
        background: var(--wine); color: #fff; border: none; border-radius: 30px;
        padding: 15px 28px; font-size: 14px; font-weight: 600; letter-spacing: 0.3px;
        box-shadow: 0 10px 26px rgba(194,37,92,0.35); z-index: 60;
      }
      ...
      .quote-type-label { text-align: left; font-size: 12.5px; color: var(--muted); margin: 4px 0 8px; }
      .quote-type-options { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 14px; }
      .quote-type-chip {
        border: 1px solid var(--gold); border-radius: 20px; padding: 8px 14px;
        font-size: 12.5px; color: var(--wine-dark); background: var(--ivory);
      }
      .quote-type-chip.active { background: var(--wine); border-color: var(--wine); color: #fff; }
      ...
      .quote-submit.ready { background: var(--wine); }
